...
//   limitations under the License.
    ...
    private function unbracket($tok) {
        ...
        if ($left || $right) {
            ...
            $right = $right = $right ?? '.';
            return [
                'tex'=>"\\left {$left} {$middle} \\right {$right}",
                'pos'=>$tok['pos'],
                'end'=>$tok['end']
            ];
        } else {
    ...
    private function strip_space($pos = 0) {
        ...
        $reduced = preg_replace('/^(\\s|\\\\(?![\\\\ ]))*/', '', $osource);
    ...
    private function intermediate_or_fraction($pos = 0) {
        ...
        if ($frac) {
            ...
            if ($second) {
                ...
                return [
                    'tex'=>"\\frac{{$ufirst['tex']}}{{$usecond['tex']}}",
                    'pos'=>$first['pos'],
                    'end'=>$second['end'],
                    'ttype'=>"fraction",
                    'numerator'=>$ufirst,
    ...
    private function matrix($pos = 0) {
        ...
        $matrix_tex = $contents["is_array"] ? "\\begin{array}{{$contents['column_desc']}} {$contents_tex} \\end{array}" : "\\begin{matrix} {$contents_tex} \\end{matrix}";
        return [
            'tex'=>"\\left {$left['tex']} {$matrix_tex} \\right {$right['tex']}",
            'pos'=>$pos,
            'end'=>$right['end'],
            'ttype'=>"matrix",
            'rows'=>$contents['rows'],
    ...
    private function matrix_contents($pos = 0, $leftright = false) {
        ...
        while (!$this->eof($end) && !($leftright ? $this->leftright_bracket($end) : $this->right_bracket($end))) {
            ...
            } elseif ($row_column_desc != $column_desc) {
                return null;
            }
            $rows[] = [
                'ttype'=>"row",
        ...
        if ($lvert) {
            ...
            if ($middle) {
                ...
                if ($rvert) {
                    ...
                    if ($second) {
                        return [
                            'tex'=>"\\left \\lvert {$middle['tex']} \\right \\rvert {$second['text']}",
                            'pos'=>$lvert['pos'],
                            'end'=>$second['end'],
                            'ttype'=>"expression",
                            'exprs'=>[$middle, $second]
    ...
    private function bracketed_expression($pos = 0) {
        ...
        if ($l) {
            ...
            if ($middle) {
                ...
                if ($r) {
                    return [
                        'tex'=>"\\left {$l['tex']} {$middle['tex']} \\right {$r['tex']}",
                        'pos'=>$pos,
                        'end'=>$r['end'],
                        'bracket'=>true,
                        'left'=>$l,
                ...
                } elseif ($this->eof($middle['end'])) {
                    return [
                        'tex'=>"\\left {$l['tex']} {$middle['tex']} \\right.",
                        'pos'=>$pos,
                        'end'=>$middle['end'],
                        'ttype'=>"bracket",
                        'left'=>$l,
        ...
        if ($left) {
            ...
            if ($middle) {
                ...
                if ($right) {
                    return [
                        'tex'=>"\\left {$left['tex']} {$middle['tex']} \\right {$right['tex']}",
                        'pos'=>$pos,
                        'end'=>$right['end'],
                        'bracket'=>true,
                        'left'=>$left,
    ...
    private function mid_expression($l, $middle, $pos) {
        ...
        if(count($middle['exprs'])==1 && $middle['exprs'][0]['ttype']=='expression') {
            ...
            $nr = $last['right'];
            return [
                'tex'=>"\\left {$l['tex']} {$firsttex} {$mid['def']['mid_tex']} {$lasttex} \\right {$nr['tex']}",
                'pos'=> $pos,
                'end'=> $end,
                'left'=> $l,
                'right'=> $nr,
                'middle'=> [
                    'tex'=> "{$firsttex} {$mid['def']['mid_tex']} {$lasttex}",
        ...
        foreach ($this->binary_symbols as $binary) {
            ...
            if ($m) {
                ...
                if ($a) {
                    ...
                    if ($b) {
                        return [
                            'tex'=>"{$binary['tex']}{$lb1}{$atex}{$rb1}{{$b['tex']}}",
                            'pos'=>$pos,
                            'end'=>$b['end'],
                            'ttype'=>"binary",
                            'op'=>$binary,
        ...
        foreach ($this->unary_symbols as $u) {
            ...
            if ($m) {
                ...
                if (isset($u['rewriteleftright'])) {
                    ...
                    if ($arg) {
                        return [
                            'tex'=>"\\left {$left} {$argtex} \\right {$right} {$sstex}",
                            'pos'=>$pos,
                            'end'=>$arg['end'],
                            'ttype'=>"unary",
                            'op'=>$m,
                        ...
                        return [
                            'tex'=>"\\left {$left} \\right {$right} {$sstex}",
                            'pos'=>$pos,
                            'end'=>$m['end'],
                            'ttype'=>"unary",
                            'op'=>$m,
    ...
    private function scientific($pos = 0) { // added
